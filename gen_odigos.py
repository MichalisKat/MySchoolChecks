...
def build():
    ...
    story += [
        ...
        Paragraph(
            'Η δεύτερη γραμμή της γραμμής εργαλείων δίνει πρόσβαση σε αυτοματισμούς '
            'που ενεργούν απευθείας στη διαδικτυακή πύλη του MySchool. '
        ...
        Paragraph(
            ...
            S['body']),
        _sp(0.2),
        Paragraph('7.2 Έλεγχος Ε.Ε.Α.', S['h2']),
        Paragraph(
            'Αυτοματοποιημένη επεξεργασία στατιστικών μαθητών με Ειδικές Εκπαιδευτικές Ανάγκες (ΕΕΑ): '
            'αυτόματη λήψη αρχείων από το MySchool, σύγκριση μεταξύ σχολικών μονάδων ή ετών, '
            'διαχωρισμός ανά σχολείο και αποστολή email με αποτελέσματα. '
            'Αναλυτικός οδηγός στο <b>Παράρτημα Β</b> στο τέλος του οδηγού.',
            S['body']),
        _sp(0.2),
        ...
        Paragraph(
            'Σύνταξη και αποστολή ενημερωτικού email προς σχολεία ή συναδέλφους. '
            'Διαθέσιμο από την 1η γραμμή εργαλείων.',
            S['body']),
        _sp(0.5),
        PageBreak(),
    ]
    ...
    # ── Παράρτημα Β — Στατιστικά Ε.Ε.Α. ─────────────────────────────────────
    story += [
        Paragraph('Παράρτημα Β — Στατιστικά Ε.Ε.Α.', S['h1']), _hr(),
        _tip(
            'Ο αυτοματισμός Ε.Ε.Α. βασίζεται σε έργο του κ. Παπαγιαννίτση Ιωάννη, '
            'με τροποποιήσεις και ενσωμάτωση στην παρούσα εφαρμογή.'
        ),
        _sp(0.3),
        Paragraph('Τι κάνει', S['h2']),
        Paragraph(
            'Η λειτουργία <b>Έλεγχος Ε.Ε.Α.</b> αυτοματοποιεί την επεξεργασία στατιστικών '
            'μαθητών με Ειδικές Εκπαιδευτικές Ανάγκες (ΕΕΑ) από το MySchool.',
            S['body']),
        _ftable([
            ('Αυτόματη λήψη',      'Κατεβάζει αρχεία Excel από το MySchool μέσω browser'),
            ('Σύγκριση περιόδων',  'Συγκρίνει στατιστικά ΕΕΑ δύο διαφορετικών χρονικών στιγμών'),
            ('Διαχωρισμός',        'Παράγει ξεχωριστό αρχείο Excel ανά σχολική μονάδα'),
            ('Αποστολή email',     'Στέλνει τα αρχεία αυτόματα στα σχολεία μέσω email'),
        ]),
        _sp(0.3),
        Paragraph('Ροή εργασίας', S['h2']),
        _ftable([
            ('Βήμα 1', 'Λήψη στατιστικών αρχείων ΕΕΑ (master / slave) από το MySchool'),
            ('Βήμα 2', 'Άνοιγμα του διαλόγου «Έλεγχος Ε.Ε.Α.» από τη γραμμή εργαλείων'),
            ('Βήμα 3', 'Επιλογή των δύο αρχείων Excel (master = νεότερο, slave = παλαιότερο)'),
            ('Βήμα 4', 'Αντιστοίχιση στηλών μεταξύ των δύο αρχείων'),
            ('Βήμα 5', 'Παραγωγή αποτελεσμάτων — ένα αρχείο Excel ανά σχολείο'),
            ('Βήμα 6', 'Προαιρετική αποστολή email στις σχολικές μονάδες'),
        ]),
        _sp(0.3),
        Paragraph('Προαπαιτούμενα', S['h2']),
        _tip(
            'Απαιτούνται δύο αρχεία Excel στατιστικών ΕΕΑ από το MySchool: '
            'ένα για την τρέχουσα περίοδο (master) και ένα για προηγούμενη '
            'περίοδο (slave). Τα αρχεία κατεβαίνουν από Στατιστικά → Ε.Ε.Α. '
            'Για την αποστολή email απαιτείται ρυθμισμένος λογαριασμός αποστολής '
            'στις Ρυθμίσεις της εφαρμογής.'
        ),
        _sp(0.3),
        Paragraph('Αποτελέσματα', S['h2']),
        _ftable([
            ('Αρχεία Excel',    'Ένα αρχείο ανά σχολείο με συγκριτικά στατιστικά ΕΕΑ'),
            ('Φάκελος εξόδου', 'Documents\\MySchoolChecksPlus\\results_YYYYMMDD\\'),
            ('Email',          'Προαιρετική αποστολή στα σχολεία (αν ρυθμιστεί SMTP)'),
        ]),
        _sp(0.5),
        PageBreak(),
    ]
