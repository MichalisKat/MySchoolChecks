...
### Versioning
- Τρέχουσα: `3.0.0`
- Επόμενη: `3.0.1` (bugfixes) ή `3.1.0` (νέα features)
...
### v4.1.0
- **Ε5 Λεπτομέρειες ωραρίου — νέο κουμπί ΑΠΟΦΑΣΗ** (δίπλα στο ΛΗΞΗ): για κάθε άτομο του excel ανοίγει την ΥΠΑΡΧΟΥΣΑ εγγραφή «Γραμματειακή Υποστήριξη» (όχι νέα), σβήνει τις Παρατηρήσεις (DXEditor7) και γράφει το κείμενο απόφασης. Προαιρετική στήλη «Απόφαση» στο excel υπερισχύει ανά γραμμή. 0 ή >1 γραμμές Γραμματειακής → παράλειψη. Backup παλιών τιμών σε Documents/MySchoolChecks/apofasi_backup_*.csv. Νέα run_decision() στο work_hours_details.py.

### v3.2.0
- **Νέο εργαλείο `absences.py` — Καταχώρηση Απουσίας σε Οργανική**: 3η κάρτα στο μενού ΔΙ.Π.Ε.Αν.Θ. («📝 Καταχώρηση Απουσίας σε Οργανική», `AbsencesDialog` στο `main.py`). Αυτόματη καταχώρηση απουσίας Ολικής Διάθεσης στην οργανική τοποθέτηση εκπαιδευτικών, μέσω Selenium.
- **Ε6 `absences.py` — ασφάλεια καταχώρησης**: (α) ο έλεγχος «ΥΠΑΡΧΕΙ ΗΔΗ» ταιριάζει τύπο ΚΑΙ ημερομηνίες Από/Έως ανά γραμμή του gridAbsences (περσινή απουσία ίδιου τύπου δεν μπλοκάρει)· (β) μετά τη συμπλήρωση ξαναδιαβάζονται τύπος/από/έως — αν κάποιο δεν ταιριάζει, CancelEdit και ΚΑΜΙΑ Αποθήκευση· (γ) μετά την Αποδοχή έλεγχος ότι η γραμμή έκλεισε (fallback UpdateEdit)· (δ) μετά την Αποθήκευση έλεγχος alert + επαλήθευση ότι η απουσία φαίνεται στο grid (reload με GET, ΟΧΙ refresh). Το «Ισχύει από» διαβάζεται πλέον από τη στήλη «Από» του αρχείου (υποχρεωτική, όπως το «Έως»). Το connect() σέβεται το BROWSER (chrome/firefox).
- **Κωδικός κλειδώματος (`_password_gate`)**: στον κώδικα υπάρχει μόνο PBKDF2-SHA256 hash (`_LOCK_SALT_HEX`/`_LOCK_ITERATIONS`/`_LOCK_HASH_HEX`). Αλλαγή κωδικού: `python tools/make_lock_hash.py`. ΠΟΤΕ ο κωδικός σε καθαρό κείμενο στο repo.
- **Λογική**: κάθε εκπαιδευτικός έχει 2 εγγραφές τοποθέτησης στο αρχείο εξαγωγής («Σχέση τοποθέτησης»). Επεξεργάζονται ΜΟΝΟ όσοι έχουν 2η εγγραφή = «Ολική Διάθεση (ανάγκες υπηρεσίας - κύριος φορέας)» — οι περιπτώσεις Απόσπασης/Επί Θητείας εξαιρούνται (χειροκίνητη καταχώρηση). Η απουσία μπαίνει στην εγγραφή της 1ης τριάδας (Οργανικά / Οργανικά σε Τμήμα Ένταξης / Οργανικά από Αρση Υπεραριθμίας), με λεκτικό «ΟΛΙΚΗ ΔΙΑΘΕΣΗ ΣΕ ΑΛΛΗ ΣΧΟΛΙΚΗ ΜΟΝΑΔΑ - Σχολικές Μονάδες Πρωτοβάθμιας», Ισχύει από σταθερό 1/9/2026, Ισχύει έως από τη στήλη Έως της εγγραφής Ολικής Διάθεσης.
- **Selenium — 2 πινακάκια στην καρτέλα εργαζομένου**: η καρτέλα έχει και το πινακάκι «Λεπτομέρειες ωραρίου εργασίας» (`gridEmplDet`) πάνω από το πινακάκι «Απουσίες» (`gridAbsences`/`gridAbs`) — το κουμπί Προσθήκη στοχεύεται ρητά με `contains(@onclick,"gridAbs.AddNewRow")` για να μην πατηθεί το λάθος πινακάκι, και το Αποδοχή με `contains(@onclick,"gridAbsences")`.
- **Selenium — IDs πεδίων ημερομηνίας**: μέσα στο grid Απουσιών, το combo `cmbAbsenceType` παίρνει δυναμικό `editnew_N` prefix, αλλά τα πεδία ημερομηνίας `DXEditor5`/`DXEditor6` (Ισχύει από/έως) ΔΕΝ το παίρνουν — σταθερά IDs `ctl00_ContentData_gridAbsences_DXEditor5_I`/`DXEditor6_I`. Η `_set_dxe_value` πλέον ελέγχεται το return value (αν το πεδίο δεν βρεθεί, εμφανίζεται μήνυμα αντί για ψευδές «✓»).
- **Ανωμαλίες → popup παράλειψης**: οτιδήποτε διαφοροποιείται από το αναμενόμενο (δεν βρέθηκε εκπαιδευτικός/γραμμή/πεδίο) εμφανίζει popup μήνυμα στον χρήστη· το κλείνει και προχωράει αυτόματα στον επόμενο εκπαιδευτικό.

### v3.1.2
- **`DipePlacementsDialog` (Επεξεργασία αρχείου τοποθετήσεων) — αυτόματο ΑΦΜ**: νέα `placements.build_afm_lookup_from_stat41()` διαβάζει το stat4_1 (Οργανικές τοποθετήσεις) με `index_col=False` (διορθώνει σιωπηλό pandas bug όπου η πρώτη στήλη ΑΜ γινόταν index και όλες οι υπόλοιπες "μετατοπίζονταν") και κάνει join ΑΜ→ΑΦΜ. Προαιρετικό πεδίο επιλογής αρχείου 4.1 στο dialog· αλλιώς αυτόματη εύρεση (`_auto_find_stat41`, ψάχνει και `MySchoolChecksPlus/downloads` και το παλιό `MySchoolChecks/downloads`). ΑΜ που δεν βρίσκονται μένουν κενά (χειροκίνητη συμπλήρωση).
- **Popup «Στοιχεία τοποθέτησης» πριν τη μετατροπή**: ζητά ΕΙΔΟΣ ΤΟΠΟΘΕΤΗΣΗΣ (dropdown με τις επίσημες τιμές MySchool) + ΑΠΟ/ΕΩΣ μία φορά, εφαρμόζονται σε όλες τις γραμμές του batch (πράσινο = συμπληρώθηκε αυτόματα).
- **Fix `PlacementsDialog` (Τοποθετήσεις — Αυτόματη Καταχώρηση)**: το `run()` διάβαζε πάντα με `header=0`, αλλά το αρχείο από την Επεξεργασία έχει banner στη γραμμή 1 και headers στη γραμμή 2 — αποτέλεσμα, καμία στήλη δεν αναγνωριζόταν και το `run()` αντικαθιστούσε όλο το αρχείο με κενές γραμμές + «Κενό ΑΦΜ». Τώρα δοκιμάζει `header=0` και `header=1` και σταματά με σαφές μήνυμα αν δεν βρεθούν οι αναμενόμενες στήλες.
- **`placements.connect(mode=...)`**: νέο popup «Θα ανοίξεις υπάρχουσα ή να δημιουργήσω νέα;» πριν τη σύνδεση (μόνο στην πρώτη εκτέλεση μέσα στο dialog). Με `mode='existing'` δεν πατάει «Προσθήκη» — περιμένει τον χρήστη να ανοίξει ο ίδιος μια υπάρχουσα απόφαση στο Chrome (χρήσιμο όταν μια καταχώρηση είχε διακοπεί στη μέση) και συνεχίζει αυτόματα μόλις εντοπίσει τη φόρμα.

### v3.0.0
- **`tmimata_genikis.py` — πλήρης επανασχεδίαση**: ο έλεγχος πλέον κατεβάζει μόνος του τα 3.1, 5.3, 5.4 (μέσα στο `run()`, μέσω `MySchoolDownloader` με τοπικό override έτους μόνο για τη δική του λήψη — δεν πειράζεται η καθολική ρύθμιση του 3.1 στο `core/downloader.py`, που το χρησιμοποιούν και άλλα εργαλεία με το τρέχον έτος). Πριν τη λήψη αλλάζει αυτόματα το ενεργό σχολικό έτος σε 2026-2027.
- **Νέα λογική**: αντί για λίστα μόνο αποκλίσεων με αποστολή email ανά σχολείο, παράγεται 1 Excel με 2 φύλλα (Δημοτικά / Νηπιαγωγεία) με **όλα** τα σχολεία — ανά τάξη Τμήματα/Μαθητές (από 3.1, Νηπιαγωγεία: ενιαία στήλη «ΠΡΟΝΗΠΙΑ-ΝΗΠΙΑ» αθροίζοντας ΝΗΠΙΑ/ΠΡΟΝΗΠΙΑ/ΠΡΟΝΗΠΙΑ-ΝΗΠΙΑ), Άθροισμα, Διαφορά Τμημάτων/Μαθητών με πράσινο/κόκκινο χρωματισμό. `HAS_EMAIL = False`, `CUSTOM_RUN = True` (ίδιο μοτίβο με `orario_pe60.py`) — main.py καλεί απευθείας `mod.run(config)`.
- **Ανθεκτικότητα στο 3.1**: οι στήλες του 3.1 εντοπίζονται πλέον με βάση το ΟΝΟΜΑ τους (όχι σταθερή θέση), γιατί ο αριθμός/σειρά στηλών αλλάζει ανάλογα με τις επιλογές ομαδοποίησης που τσεκαρίστηκαν κατά τη λήψη (π.χ. αν λείπει η «Τύπος Σχολείου»).
- Version bump: `config.py`, `myschool-checks.nsi`, `compile_installer.bat` → 3.0.0.

### v2.9.5
- **Νέος έλεγχος `orario_pe60.py` — Υποχρεωτικό Ωράριο ΠΕ60 (Νηπιαγωγεία)**: μενού «🗓 Νέο Σχ. Έτος» → «🕐 Υποχρεωτικό Ωράριο ΠΕ60». Φιλτράρει το 2.1 σε Είδος=Νηπιαγωγεία και χωρίζει σε 2 κατηγορίες βάσει Λειτουργικότητας (Κατηγορία 1: 1-3 → Υποχρεωτικό Ωράριο πρέπει να είναι 25· Κατηγορία 2: ≥4 → πρέπει να είναι διάφορο του 25). Εκπαιδευτικοί ΠΕ60/ΠΕ60.50 αντλούνται από 4.1 (Οργανικές τοποθετήσεις) + 4.2 (Αποσπασμένοι, προαιρετικό· υπερισχύει αν έχει έγκυρο κωδικό) — στήλη ελέγχου «Υποχ. Ωράριο» (το πραγματικό Υποχρεωτικό Διδακτικό Ωράριο, όχι οι ώρες υπηρέτησης στο φορέα). Αποτέλεσμα: 1 φύλλο Excel μόνο με τις αποκλίσεις, χωρίς αποστολή email. Τοποθετήσεις εκτός λίστας Νηπιαγωγείων 2.1, με άγνωστη Λειτουργικότητα, ή χωρίς τιμή ωραρίου αγνοούνται σιωπηλά.
- `CHECKS_EXCLUDED` (main.py) ενημερώθηκε με `'orario_pe60'` — ο έλεγχος καλείται μόνο από το μενού Νέο Σχ. Έτος, όχι από τη γενική λίστα ελέγχων.

### v2.9.2
- **Νέος έλεγχος `tmimata_genikis.py` (2026-2027)**: έλεγχος τμημάτων γενικής παιδείας / δυναμικού για νηπιαγωγεία (5.3) και δημοτικά (5.4) — σύγκριση Λειτουργικότητας vs Υποχρεωτικά Πρωινά Τμήματα / Τμήματα Γενικής Παιδείας και αντίστοιχα με Μαθητές — αποτέλεσμα: σχολεία με απόκλιση, με στήλες Μαθητές, Διαφορά Τμημάτων/Μαθητών και email αποστολής (από stat2_2)
- **Λήψη 5.3 / 5.4**: νέα στατιστικά αποτύπωσης νηπιαγωγείων και δημοτικών — αυτόματη αλλαγή σχολικού έτους σε 2026-2027 πριν τη λήψη (μέσω cmbActiveAcadYear στο Default.aspx)
...
- `framework.py` `get_downloaded_file()`: **διορθώθηκε path** — χρησ�TESTMARKER123
� `Documents\MySchoolChecksPlus\` (όχι LOCALAPPDATA)
